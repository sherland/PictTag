...
public class ExifToolSidecarWriterTests : IDisposable
{
    ...
    public async Task WriteSidecarAsync_WritesSubjectKeywordsAndRegions()
    {
        ...
        // Iptc4xmpExt:ImageRegion is written alongside -RegionInfo/mwg-rs:Regions, with the
        // *top-left* corner (unlike MWG's center point) - verified empirically against
        // exiftool's own maintained MWG<->IPTC region conversion logic. cat box -> rbX=0.2, rbY=0.1.
    ...
    public async Task WriteSidecarAsync_LabelWithSpecialCharacters_RoundTripsCorrectly()
    {
        ...
        Assert.Equal(trickyLabel, xmp.GetLocalizedText(XmpNamespaces.IptcExt, "ImageRegion[1]/Iptc4xmpExt:Name", "", "x-default").Value);
    }
